...
double* BWF_Fitter::Fit(double* initialGuess, bool weightedFit)
{
	if(!_paramsSet) {std::cout << "Attempt to use BWF_fitter without setting survival params. Failure." << std::endl; return nullptr; }
	if(!_functionSet) {std::cout << "Attempt to use BWF_fitter without setting biological weighting function to fit. Failure." << std::endl; return nullptr; }

	return GeneralizedBWFFitting(_survivalParams, _fitFunc, initialGuess, weightedFit);
}

int BWF_Fitter::GeneralizedBWF(const gsl_vector* x, void* data, gsl_vector* f)
{
...
double* BWF_Fitter::GeneralizedBWFFitting(CellStudySurvivalParameters& survivalParams, BiologicalWeightingFunction fitFunc, double* initialGuess, bool weightedFit)
{
    ...
    int nFittingParams = fitFunc.GetNumFittingParams();
