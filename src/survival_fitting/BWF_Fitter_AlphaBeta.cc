...
double* BWF_Fitter_AlphaBeta::Fit(double* initialGuess, bool weightedFit)
{
	...
	if(!_functionSet) {std::cout << "Attempt to use BWF_Fitter_AlphaBeta without setting biological weighting function to fit. Failure." << std::endl; return nullptr; }
...
int BWF_Fitter_AlphaBeta::GeneralizedBWF(const gsl_vector* x, void* data, gsl_vector* f)
{
	...
	const int nParams = (fittingFunction->GetNumFittingParams()+2); //Extra parameter is beta
	...
	for(const std::pair<std::string,TH1D>& spectrumPair:*pDySpectra) //First we iterate over each of the lineal energy spectra
	{
		...
		for(int k = 1; k <= dySpectrum.GetNbinsX(); ++k) //Iterate over every histogram bin to calculate alpha
		{
			...
			betaPredicted += ((params[nParams-1]*center)+params[nParams-2])*value*width;
		}
...
double* BWF_Fitter_AlphaBeta::GeneralizedBWFFitting(CellStudySurvivalParameters& survivalParams, BiologicalWeightingFunction fitFunc, double* initialGuess, bool weightedFit)
{
    ...
    int nFittingParams = (fitFunc.GetNumFittingParams()+2);
