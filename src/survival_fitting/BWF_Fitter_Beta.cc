...
double* BWF_Fitter_Beta::Fit(double* initialGuess, bool weightedFit)
{
	if(!_paramsSet) {std::cout << "Attempt to use BWF_Fitter_Beta without setting survival params. Failure." << std::endl; return nullptr; }
	if(!_functionSet) {std::cout << "Attempt to use BWF_Fitter_Beta without setting biological weighting function to fit. Failure." << std::endl; return nullptr; }

	return GeneralizedBWFFitting(_survivalParams, _fitFunc, initialGuess, weightedFit);
}

int BWF_Fitter_Beta::GeneralizedBWF(const gsl_vector* x, void* data, gsl_vector* f)
{
	//Load in the dySpectra and underlying ground truth alpha values
	const std::vector<std::pair<std::string,TH1D>>* const pDySpectra = &(((CellStudySurvivalParameters*)data)->dySpectra);
	const std::vector<std::vector<double>>* const sf = &(((CellStudySurvivalParameters*)data)->survivingFraction);
	const std::vector<std::vector<double>>* const dose = &(((CellStudySurvivalParameters*)data)->dose);
	//const double* beta = &(((CellStudySurvivalParameters*)data)->beta);

	const BiologicalWeightingFunction* fittingFunction = &(((CellStudySurvivalParameters*)data)->fittingFunction);
	const int nParams = (fittingFunction->GetNumFittingParams()+1); //Extra parameter is beta
	...
	//This is a reference just to save computational time
	double& beta = params[nParams-1];
	...
	for(const std::pair<std::string,TH1D>& spectrumPair:*pDySpectra) //First we iterate over each of the lineal energy spectra
	{
		...
		for(int k = 1; k <= dySpectrum.GetNbinsX(); ++k) //Iterate over every histogram bin to calculate alpha
		{
			...
			double ryVal = fittingFunction->GetValue(params,center);
...
double* BWF_Fitter_Beta::GeneralizedBWFFitting(CellStudySurvivalParameters& survivalParams, BiologicalWeightingFunction fitFunc, double* initialGuess, bool weightedFit)
{
	survivalParams.fittingFunction = fitFunc;
    ...
    int nFittingParams = (fitFunc.GetNumFittingParams()+1);
